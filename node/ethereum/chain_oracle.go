...
type ChainOracle interface {
	ProvideShares(rblock common.Hash, pointerIndex uint8, shareProof *chainOracleContract.SharesProof, blockProofs []chainOracleContract.BinaryMerkleProof) (*types.Transaction, error)
...
func (c *Client) ProvideShares(rblock common.Hash, pointerIndex uint8, shareProof *chainOracleContract.SharesProof, blockProofs []chainOracleContract.BinaryMerkleProof) (*types.Transaction, error) {
	transactor, err := c.transactor()
	if err != nil {
		return nil, fmt.Errorf("failed to create transactor: %w", err)
	}
	return c.chainLoader.ProvideShares(transactor, rblock, pointerIndex, *shareProof, blockProofs)
}
