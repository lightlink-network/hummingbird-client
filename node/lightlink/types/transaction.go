...
func (tx *Transaction) EffectiveGasTip(baseFee *big.Int) (*big.Int, error) {
	...
	if tx.GasTipCap().Cmp(effectiveTip) < 0 {
		return tx.GasTipCap(), err
	}
	return effectiveTip, err
}
...
// IsEnterprise returns true if the transaction is an enterprise tx type.
//
// We have to check is not a deposit tx as deposit txs also have gasPrice = 0
// therefore also return as enterprise tx is not checked // TODO: add enterprise tx type
func (tx *Transaction) IsEnterpriseTx() bool {
	return tx.GasPrice().Cmp(big.NewInt(0)) == 0 && !tx.IsDepositTx() && !tx.IsDepositTxV2()
}
...
func (tx *Transaction) SourceHash() common.Hash {
	if dep, ok := tx.inner.(*DepositTxV2); ok {
...
func (tx *Transaction) IsDepositTxV2() bool {
	return tx.Type() == DepositTxV2Type
}
