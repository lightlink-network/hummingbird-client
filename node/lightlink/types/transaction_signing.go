...
type lightlinkSigner struct{ EIP155Signer }

// lightlinkSigner returns a signer that accepts EIP-2930 access list transactions,
// EIP-155 replay protected transactions, and legacy Homestead transactions.
func NewLightLinkSigner(chainId *big.Int) Signer {
	return lightlinkSigner{NewEIP155Signer(chainId)}
}

func (s lightlinkSigner) ChainID() *big.Int {
	return s.chainId
}

func (s lightlinkSigner) Equal(s2 Signer) bool {
	x, ok := s2.(lightlinkSigner)
	return ok && x.chainId.Cmp(s.chainId) == 0
}

func (s lightlinkSigner) Sender(tx *Transaction) (common.Address, error) {
	V, R, S := tx.RawSignatureValues()
	switch tx.Type() {
	case LegacyTxType:
		if !tx.Protected() {
	...
	case DepositTxV2Type:
		...
		case *DepositTxV2:
			return tx.inner.(*DepositTxV2).From, nil
		case *depositTxV2WithNonce:
			return tx.inner.(*depositTxV2WithNonce).From, nil
		}
	default:
...
func (s lightlinkSigner) SignatureValues(tx *Transaction, sig []byte) (R, S, V *big.Int, err error) {
	...
	case *DepositTx:
		...
		V = big.NewInt(int64(sig[64]))
	case *DepositTxV2:
		return nil, nil, nil, fmt.Errorf("v2 deposits do not have a signature")
	default:
...
// Hash returns the hash to be signed by the sender.
// It does not uniquely identify the transaction.
func (s lightlinkSigner) Hash(tx *Transaction) common.Hash {
	switch tx.Type() {
	case LegacyTxType:
	...
	case DepositTxV2Type:
		panic("v2 deposits cannot be signed and do not have a signing hash")
	default:
