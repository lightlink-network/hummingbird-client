...
// Test isEnterprise checks that the isEnterprise method returns correct
func TestIsEnterpriseTx(t *testing.T) {
	tests := []struct {
		name string
		tx   *Transaction
		want bool
		...
		{
			name: "legacy tx",
			tx:   NewTx(&LegacyTx{GasPrice: big.NewInt(1)}),
			want: false,
		},
		{
			name: "deposit tx",
		...
		{
			name: "deposit tx gas price 0",
			tx:   NewTx(&DepositTx{GasPrice: big.NewInt(0)}),
			want: false,
		},
		{
			name: "enterprise tx",
			tx:   NewTx(&LegacyTx{GasPrice: big.NewInt(0)}),
			want: true,
		},
		{
			name: "enterprise tx gas price nil",
			tx:   NewTx(&LegacyTx{}),
			want: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := tt.tx.IsEnterpriseTx(); got != tt.want {
