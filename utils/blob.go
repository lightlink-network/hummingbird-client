...
func RawIndexToSharesIndex(rawIndex int, s []shares.Share) (share int, shareIndex int) {
	...
	// rawCursor := 0 // keeps a track of the amount of share data we have read
	// for i := 0; i < len(s); i++ {
	// 	shareDataSize := len(s[i].ToBytes()[ShareDataStart(s[i]):])
	// 	if rawIndex < rawCursor+shareDataSize {
	// 		shareIndex = rawIndex - rawCursor
	// 		fmt.Printf("# rawIndex: %d, rawCursor: %d, shareDataSize: %d\n", rawIndex, rawCursor, shareDataSize)
	// 		fmt.Printf("# shareIndex: %d\n", shareIndex)
	// 		return i, shareIndex
	// 	}
	// 	fmt.Printf("> rawIndex: %d, rawCursor: %d, shareDataSize: %d\n", rawIndex, rawCursor, shareDataSize)
	// 	rawCursor += shareDataSize
	// }
	// return
}
