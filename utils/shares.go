...
import (
	...
	orpc "github.com/celestiaorg/celestia-openrpc/types/share"
...
func BytesToBlob(ns string, buf []byte) (*blob.Blob, error) {
	// get the namespace
	_ns, err := libshare.NewNamespace(0, []byte(ns))
	if err != nil {
		return nil, err
	}
...
func NSSharesToShares(ns orpc.NamespacedShares) []shares.Share {
	s := []shares.Share{}
